...
public class SimpleSiteService implements SiteService, UserDetailsService {
    ...
    private final int randomShortUrlLine = 7;

    @Override
    public String convertURL(String site) {
        String convertedURL = RandomStringUtils.random(randomShortUrlLine, true, true);
        return convertedURL;
    }
    ...
    public Site fillSiteByCredentials(String site) {
        ...
        boolean flag = false;
        while (!flag) {
            generatedLogin = RandomStringUtils.random(randomStringLength, true, true);
            generatedPassword = RandomStringUtils.random(randomStringLength, true, true);
